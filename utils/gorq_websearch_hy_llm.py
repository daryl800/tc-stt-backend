...
def extract_info_withLLM(text: str) -> MemoryItem:
    try:
        ...
        system_prompt = f"""
            ...
            5. 請在用戶輸入的內容中萃取 (category)， 為以下類別的其中一類：一般、家庭、健康、醫療、運動、出行、工作、學習、音樂、娛樂、約會、重要、危險 或 任務。
            6. (categoryIcon) 必須完全反映 (category)，並以適當(emoji)代表，絕不能留空。
            7. 若 (category) 是與藥物有關的內容，其(categoryIcon)應該為💊。
        ...
        response = dashscope.Generation.call(
            model=QIANWEN_LLM_MODEL_FLASH,
            messages=messages,
            temperature=0.7,
            max_tokens=300
        )

        raw_content = response.output.text.strip()
        ...
        print(f"[DEBUG] reminder_date: {reminder_date}")
